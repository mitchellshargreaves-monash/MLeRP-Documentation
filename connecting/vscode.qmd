...
For MLeRP QCIF:
```bash
ProxyCommand="ssh <username>@{{< var qcif_login >}} /usr/local/sv2/sshnc.sh"
```
...
For MLeRP QCIF:
```bash
ProxyCommand="ssh.exe <username>@{{< var qcif_login >}} /usr/local/sv2/sshnc.sh"
```
...
Note that you will need to authenticate with your ssh key/cert, either through your authorized keys file [or by generating a cert with ssossh](/connecting/ssh.qmd#ssossh). You can then add it to your agent or provide it to the ssh command (and proxy command) with the `-i` flag.
...
You'll then be asked to enter the SSH connection information. Make sure you replace `<username>` with your {{< var cluster_name >}} username and `<ProxyCommand>` with the [proxy command](/connecting/vscode.qmd#introduction) appropriate to your OS and selected region.
