...
```
Host MLeRP_monash_login
	HostName {{< var monash_login >}}
	User <User>
	IdentityFile </path/to/ssh/key>

Host MLeRP_monash_job
	HostName VSCode
	User <USER>
	IdentityFile </path/to/ssh/key>
	ProxyCommand ssh.exe -i </path/to/ssh/key> <User>@{{< var monash_login >}} /apps/strudel2/strudel_apps/sshnc.sh
```

This will give you two configurations:

1. `MLeRP_Monash_login`  
    For connecting directly to the login node for simple administrative tasks

2. `MLeRP_Monash_job`  
    For connecting to a 'Jupyter Lab' or 'Terminal' job that you launch from Strudel2. If it can't find one, it will create a Panther QoS job with 4 VCPUs and 16 GB RAM.

```bash
ssh MLeRP_Monash_login
ssh MLeRP_Monash_job
```
